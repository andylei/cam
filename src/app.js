...
let svgBody = d3.select("svg#viz-body"),
  svgHeader = d3.select("svg#viz-header"),
  margin = { top: 40, right: 20, bottom: 40, left: 0 },
  width = +svgBody.attr("width") - margin.left - margin.right;

let container = svgBody
  .append("g")
  .attr("transform", "translate(" + margin.left + "," + 0 + ")");
let headerContainer = svgHeader
  .append("g")
...
function toggleFilter() {
  document
    .getElementById("control-filter-container")
  ...
  document.addEventListener("click", function (e) {
    ...
    while (element) {
      ...
      if (
        ...
        element.classList.contains("d3-tip")
      ) {
        return;
      }
...
function serializeConfig() {
  ...
  if (items && items.length) {
    configItems.push(
      "items=" + CONFIG.filterItems.map((i) => DATA[i].id).join(",")
    );
  }
...
function buildHeightAndYAxis(showWeights, data) {
  ...
  let y = d3
    .scaleLinear()
    .range([0, BAR_HEIGHT])
    .domain([0, d3.min(data, prop("weight"))]);
  return {
...
function rerender() {
  ...
  const closeBtn =
    '<span class="hide-popover"><button onclick="closeToolTip()">X</button></span>';
  ...
  toolTip = d3
    ...
    .html((d) => {
      let rows = [
        ...
        `Usable range: ${renderNum(d.usable_lower)} - ${renderNum(
          d.usable_upper
        )} ${unit}`,
      ].map((row) => `<p>${row}</p>`);
  ...
  let x =
    CONFIG.scale === "linear" ? d3.scaleLinear() : d3.scaleLog().base(2).nice();
  x = x.rangeRound([0, width]);

  x.domain([
    units(d3.min(data, prop(lowerProp))),
    units(d3.max(data, prop(upperProp))),
  ]);

  let frameHeight = height + margin.top + margin.bottom + 2 * INNER_PADDING;
  svgBody.attr("height", frameHeight + "px");

  container.selectAll("g").remove();
  headerContainer.selectAll("g").remove();
  ...
  container
    .append("g")
    .attr("class", "grid sub-x")
    .call(
      d3
        .axisBottom(x)
        .ticks(40)
        .tickSize(height + 2 * INNER_PADDING)
        .tickFormat("")
    );
